...
- `memory/memory.go` — Core `Store` type. Manages `.ctxlog/` directory with per-shard JSONL files. Uses BSD `flock` for cross-process file locking (exclusive for writes, shared for reads).
